/*
   TGrep2 - A program that searches for patterns in a corpus of trees.

   Copyright (C) 2001-2005  Douglas Rohde
   Questions or bug reports should be addressed to dr+tg@tedlab.mit.edu
   ...
   Thanks to Julia Hockenmaier for encouraging support for Combinatory
   Categorial Grammar (CCG) and for implementing a first draft.
 */
...
#define COOKIE_CCG "Tgrep2 CCG Corpus File"
#define NO_TREE "<none>"
...
char CCGMode = 0;          /* True for Combinatory Categorial Grammar mode,
                              which has a different set of characters for
                              parse trees and search patterns. */
...
FILE *openCorpusFile(char *filename) {
  ...
  FILE *corpus = fatalReadFile(filename);
  String S = newString(32, MEM_STRING);

  readBinString(corpus, S);
  if (!strcmp(S->s, COOKIE1)) version = 1;
  else if (!strcmp(S->s, COOKIE2)) version = 2;
  else if (!strcmp(S->s, COOKIE_CCG)) {
    version = 2;
    setCCGMode();
...
int readLexicon(FILE* corpus) {
  ...
  readBinInt(corpus, &NumSentences);
  freeString(S);
  freeString(N);
  return NumWords;
}
...
Link parsePatternLinks(char **sp) {
  ...
  skipBlank(sp);
  s = *sp;
  while (*s && *s != CloseTreeChar && *s != CloseLogicChar &&
         *s != ';' && *s != ':') {
    if (L->numLinks) {
      if (*s == '|') {
        L = O;
...
Node parsePatternNode(char **sp, char top) {
  ...
  while (**sp == OpenTreeChar || **sp == '`') {
    if (**sp == '`') print = TRUE;
    else if (**sp == OpenTreeChar) depth++;
    (*sp)++;
    skipBlank(sp);
  }
...
void printPattern(Node P, int depth) {
  if (P->link) {fputc(OpenTreeChar, stderr); depth++;}
...
char parseTree(FILE *file, Sentence S) {
  ...
  if (c != OpenTreeChar) 
    fatalError("Tree %d doesn't start with %c.",
               SentenceNum + 1, OpenTreeChar);
...
void prepareCorpus(char *infile, char *outfile) {
  ...
  else writeBinString(out, COOKIE2, -1);
...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  while ((opt = getopt(argc, argv, "?ab:c:CdefghiKlm:p:r:s:tuvwxz")) != -1) {
    switch (opt) {
    case 'a':
      TMatchMode = M_ALL;
      break;
    ...
    case 'p':
      if (argc < optind + 1)
        fatalError("The -p flag requires two arguments.\n"
                   "For help, try: %s -h", argv[0]);
  ...
  /* Open the corpus file and load the words.  This is done early to set the
     CCG mode, if necessary. */
